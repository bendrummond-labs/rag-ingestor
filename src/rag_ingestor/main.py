...
async def ingest(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="File encoding is not UTF-8"
        )
    ...
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.post(
                EMBEDDER_URL, json={"file_id": file_id, "chunks": chunks}
            )
        ...
        logger.error(
            f"Embedder returned error {e.response.status_code} for file ID: {file_id}"
        )
